...
;   - the CBM screen-editor input -> a plain line editor (RDLINE)
;   - variables moved to $0300 (the page the ROM reserves for programs);
;     zero page remapped into the .prg window ($40-$63)
;   - L/S talk to the host filesystem through the ROM (raw bytes, no
;     device numbers); V (verify) dropped
;   - @ passes the rest of the line to the K/OS shell: @DIR, @CD PRG ...
;   - X returns to the shell with RTS
; The assembler, disassembler, monitor logic and tables are Butterfield's.
...
CHROUT_ = $FF80             ; A = char (clobbers registers; use MCHROUT)
GETIN_  = $FF86             ; -> A, 0 if none (registers too; use MGETIN)
LOADRM  = $FF89             ; name ptr $F0/F1, dest $F2..F5 -> A = status
SAVERM  = $FF8C             ; name ptr $F0/F1, src $F2..F5, len $F6..F9 -> A
SHELL_  = $FF8F             ; A/X = command line pointer
KBRK    = $D103             ; break peek: pending ESC/Ctrl-C, removed+returned

; -----------------------------------------------------------------------------
; variables: the page the ROM reserves for programs ($0300-$043F).
; EQUs, not .FILLs, so the assembled image stays one contiguous chunk.
VARBASE = $0300             ; 148 bytes used
ACMD    = VARBASE+$00
LENGTH  = VARBASE+$01
MNEMW   = VARBASE+$02
SAVX    = VARBASE+$05
...
; -----------------------------------------------------------------------------
; entry: the shell JSRs here.  No BRK frame to unwind -- clear the
; register shadows, remember the stack, fall into the main loop.
SUPER   LDY #MSG4-MSGBAS    ; banner
        JSR SNDMSG
        LDA #0
        STA NDX             ; nothing pending in the prefill buffer
        LDX #6              ; clear the PCH..SP register shadows
SUPCLR  STA PCH,X
        DEX
        BPL SUPCLR
        TSX
        STX SP              ; the honest stack pointer
        STX SAVSP           ; and the one X restores to leave
        JMP STRT
...
STRT    JSR CRLF            ; new line
        LDA #"."            ; the monitor's prompt
        JSR MCHROUT
        JSR RDLINE          ; read a line into INBUFF (NUL-terminated)
        LDX #0
        STX CHRPNT
ST1     JSR GETCHR          ; get a character from the buffer
        ...
        BEQ ST1
S0      CMP #"a"            ; commands may be typed in either case
        BCC S0UP            ;   (the tables hold upper); hunt strings and
        CMP #"z"+1          ;   filenames pass through GETCHR untouched
        BCS S0UP
        AND #$DF
S0UP    LDX #KEYTOP-KEYW    ; loop through valid command characters
...
; -----------------------------------------------------------------------------
; dispatch command
S2      CPX #$0B            ; @ takes the raw rest of the line:
        BNE S2A             ;   no parameter parse, straight to the handler
        JMP DSTAT
S2A     CPX #$13            ; the last commands in the table are load/save
...
; -----------------------------------------------------------------------------
; exit monitor [X]: back to the K/OS shell
EXIT    LDX SAVSP           ; the stack as it was on entry
        TXS
        RTS                 ; run_at's trampoline takes it from here
...
ALTMX   JSR DISPMEM         ; re-display the line to make ascii match hex
        JMP STRT            ; back to main loop
...
DCHAR   LDA (TMP2),Y        ; load byte at start address + Y
        CMP #$20            ; printable ASCII is $20-$7E
        BCC DDOT
        CMP #$7F
        BCC DCHROK
DDOT    LDA #$2E            ; everything else prints as '.'
DCHROK  JSR MCHROUT
        INY                 ; next byte
        CPY #8              ; have we output 8 bytes yet?
...
; -----------------------------------------------------------------------------
; load or save [LS] -- the host filesystem through the ROM, raw bytes.
;   L "NAME" ADDR         load the file's bytes at ADDR
;   S "NAME" START END    save START up to (not including) END
LD      LDY #0
L1      JSR GETCHR          ; get a character
        BEQ LERROR          ; no filename given
        CMP #$20            ; skip leading spaces
        BEQ L1
        CMP #$22            ; filename must start with a quote
        BNE LERROR
LNAME   JSR GETCHR          ; collect the name
        BEQ LERROR          ; line ended inside the name
        CMP #$22            ; closing quote?
        BEQ LGOT
        STA STAGE,Y
        INY
        CPY #ESTAGE-STAGE-1 ; room for the NUL
        BCC LNAME
LERROR  JMP ERROR
LGOT    LDA #0              ; NUL-terminate the name
        STA STAGE,Y
        LDA #<STAGE         ; name pointer for the ROM
        STA $F0
        LDA #>STAGE
        STA $F1
        JSR GETPAR          ; the address (load) / start (save)
        BCS LERROR
        LDA TMP0
        STA $F2
        LDA TMP0+1
        STA $F3
        LDA #0              ; the machine is 28-bit; the monitor speaks 16
        STA $F4
        STA $F5
        JSR COPY12          ; start to TMP2 as well (for the save length)
        JSR GETPAR          ; end address (save only)
        BCS LTRYL           ; none: it must be a load
        LDA SAVY
        CMP #"S"
        BNE LERROR
        JSR SUB12           ; TMP0 = end - start
        BCC LERROR          ; end before start
        LDA TMP0            ; the length for the ROM
        STA $F6
        LDA TMP0+1
        STA $F7
        LDA #0
        ...
        STA $F9
        JSR SAVERM          ; A = status
        ORA #0              ; the stub's bank restore leaves its own flags
        BNE LSVERR
LSVXIT  JMP STRT
LTRYL   LDA SAVY
        CMP #"L"
...
A1BYTE  LDA OPCODE          ; put opcode into instruction
        STA (TMP2),Y
        JSR CRLF            ; carriage return (no cursor-up on this console)
        ...
        LDA #"A"            ; prefill the next line with "A XXXX "
        STA PENDBUF         ;   so Return assembles at the next address
        LDA #" "
        STA PENDBUF+1
        STA PENDBUF+6
        ...
        STA PENDBUF+4
        STX PENDBUF+5
        LDA #7              ; set number of prefilled characters
        STA NDX
        JMP STRT            ; back to main loop
...
CHOUT   CMP #$0D            ; output char with special handling of CR
        BNE FLIP
CRLF    LDA #$0D            ; the console treats CR as a newline
FLIP    JMP MCHROUT
...
WRTWO   STX SAVX            ; save X
        ...
        JSR MCHROUT          ; output upper nybble
        TXA                 ; transfer lower to A
        LDX SAVX            ; restore X
        JMP MCHROUT          ; output lower nybble
...
NZERO   INC DIGCNT          ; increment digit counter
        ...
        JSR MCHROUT          ; output character
ZERSUP  DEX                 ; decrement number of leading zeros
        BNE DIGOUT          ; next digit
        RTS

; -----------------------------------------------------------------------------
; shell escape [@]: the rest of the line goes to the K/OS shell.
; "@" alone lists the directory; "@CD PRG", "@DIR A", "@TYPE F" all work.
; Dispatched before the parameter parse (S2), so CHRPNT sits just past "@".
DSTAT   LDX CHRPNT
        LDY #0              ; copy the tail to STAGE, NUL-terminated
DSH1    LDA INBUFF,X
        STA STAGE,Y
        BEQ DSH2
        INX
        ...
        LDA #0
        STA STAGE,Y
DSH2    CPY #0              ; an empty tail means DIR
        BNE DSGO
DSCPY   LDA DIRTXT,Y
        STA STAGE,Y
        BEQ DSGO
        ...
        BNE DSCPY
DSGO    JSR CRLF
        LDA #<STAGE         ; A/X = the line, as the ROM expects
        LDX #>STAGE
        JSR SHELL_
        JMP STRT
DIRTXT  .TEXT "DIR"
...
; -----------------------------------------------------------------------------
; print and clear routines (the CBM screen-editor clear is a no-op here)
CLINE   JMP CRLF
SNDCLR  JMP SNDMSG
SNCLR   RTS
...
; assumes the C64 KERNAL's register-preserving contract.
MCHROUT STA SAVA
        PHX
        PHY
        JSR CHROUT_
...
MGETIN  PHX                 ; -> A (0 if none), X/Y preserved
        PHY
        JSR GETIN_
        PLY
        PLX
        ORA #0              ; the flags belong to A
        RTS

; the C64 STOP contract: Z=1 when the stop key (here ESC/Ctrl-C) was hit
STOP    LDA KBRK            ; reading removes a pending break from the queue
        BNE STOPHIT
        LDA #1              ; Z=0: run on
        RTS
STOPHIT LDA #0              ; Z=1: stop
        RTS

; read one line into INBUFF, NUL-terminated, echoing as we go.
; PENDBUF/NDX prefill the line (the assembler's auto-advance).
RDLINE  LDX #0
        LDY NDX             ; anything pending?
        BEQ RDKEY
RDPRE   LDA PENDBUF,X       ; echo and take the prefill first
        STA INBUFF,X
        JSR MCHROUT
        INX
        CPX NDX
        ...
        STA NDX
RDKEY   JSR MGETIN
        BEQ RDKEY           ; poll until a key arrives
        CMP #$0D            ; return ends the line
        BEQ RDDONE
        CMP #$08            ; backspace rubs out
        BEQ RDBS
        CMP #$20            ; only printable characters land
        BCC RDKEY
        CMP #$7F
        BCS RDKEY
        CPX #ENDIN-INBUFF-1 ; leave room for the NUL
        BCS RDKEY
        STA INBUFF,X
        INX
        JSR MCHROUT         ; echo
