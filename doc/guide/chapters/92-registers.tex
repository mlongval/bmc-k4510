...
The consequence is that the voice changes here. These are working
comments from working code: terse, occasionally opinionated, and using
the machine's own shorthand --- \verb|LE| for little-endian, \verb|R:|
and \verb|W:| for a register that reads and writes differently, \verb|$|
for hex. Addresses are given as an offset inside the device's block
where the block's base is obvious, and in full where it is not.
