...
\item[Video] border width and colour around the picture; the screen font
  ...
  which is how the picture is fitted to the window: \emph{sharp} (each
  pixel a hard square), \emph{soft} (smoothed), or \emph{sharp-fit} (a
  whole-number multiple, then smoothed for whatever is left over --- crisp
  text at any window size, which neither of the others manages); full
  screen.
