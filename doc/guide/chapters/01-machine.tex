...
\item \textbf{CPU: the 45GS10} --- the MEGA65's 45GS02 instruction set (a
  ...
  host can hold; the design's direction is for the machine to measure
  that itself at boot. \verb|INFO| and the banner report the clock in
  force, which is the only honest number.
...
\item[Machine] \emph{save state} and \emph{load state}, four slots
  ...
  starts at 40.5 and the Pi at 15, and both are starting points rather
  than facts about the machine. A clock is right for a host when it
