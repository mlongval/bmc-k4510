...
\begin{aside}
\textbf{Use an SDHC card (32\,GB or less).} The Pi's boot ROM reads only
FAT32, and SDHC cards are FAT32 from the factory --- they just work. A
bigger SDXC card (64\,GB+) ships exFAT and will \emph{not} boot until
reformatted; \verb|./install-sd.sh zip /dev/sdX --format| does that (and
erases the card). The machine needs about 5\,MB --- any old card is
plenty.
\end{aside}
