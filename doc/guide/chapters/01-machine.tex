...
\item \textbf{CPU: the 45GS10} --- the MEGA65's 45GS02 instruction set (a
  ...
  number, and the Pi at 15) and \verb|BENCH| measures which one your
  host can hold. (Having the machine measure that itself at boot is
  proposed in \pth{docs/CPU-CLOCK-POLICY.md}, and not built.)
  \verb|INFO| and the banner report the clock in force, which is the
  only honest number.
...
\item[Machine] \emph{save state} and \emph{load state}, four slots
  ...
  (202.5, 162, 121.5, 81, 60, 40.5, 30, 20, 15, 10); the desktop starts
  at 40.5 and the Pi at 15, and both are starting points rather than
  facts about the machine. A clock is right for a host when it
