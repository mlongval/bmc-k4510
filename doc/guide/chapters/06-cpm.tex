...
like. \texttt{/CPM/A/0/K-TURBO.SUB} is the pattern:
...
the shell; \texttt{/SYSTEM/STARTUP.SAMPLE} defines that one, \verb|WS| and
\verb|MBASIC|.
...
The cursor keys do nothing in CP/M software. They reach the Z80 as the
VT100 sequences a terminal sends, and the programs of 1984 were not
written to read those: they use the WordStar keys --- Ctrl-E, S, D and X
for up, left, right and down --- and so does Turbo Pascal
(Chapter~\ref{cha:pascal}). That is CP/M being CP/M, not a fault of the
Tube.
