...
\section{Sprites}
VICKe has 128 hardware sprites, and BBC BASIC reaches them the way RISC OS
...
\texttt{INVADERS.BBC} and \texttt{TUNNEL.BBC} in \texttt{/BBCBASIC} are
...
current directory, so \verb|CD BBCBASIC| then \verb|BBC| lets
\verb|LOAD "KALEID.BBC"| work with no directory prefix; the machine's
...
prints as text. On the Pi both co-processors run on core 3, the
second processor.
\end{aside}
