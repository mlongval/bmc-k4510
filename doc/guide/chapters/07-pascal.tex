...
it cannot see VICKY, the SIDs or anything else on the K4510 side of the
...
\noindent \verb|TURBO| is an alias that \pth{/SYSTEM/STARTUP.SAMPLE}
...
Ctrl-K then D leaves the editor for the menu, and the cursor keys do
nothing --- they reach the Z80 as VT100 sequences that the software of
the time was never written to read. \verb|R| runs the program in memory;
...
is a \texttt{.prg} in \pth{fs/PRG} that the shell's \verb|RUN| loads like
any other program. It is Tomasz Biela's compiler (MIT), a
...
typed after its name. All three are already compiled and in
\pth{fs/PRG}, so you can run them without installing anything.
...
\item \verb|uses k4510| gives every chip as a typed variable at its
  address --- \verb|VICKY[reg]|, \verb|SIDREG[]|, \verb|DMA_SRC|,
