...
/* RAM under the ROM (K-05): bank blocks 5-7 onto the physical RAM under it;
 * the I/O page and the stub page $FF00 stay, so every ROM call keeps working.
 * $0800-$CFFF + $E000-$FEFF = 62 KB. */
static void rom_out(void) { bank_set(5, 0xA000UL); bank_set(6, 0xC000UL); bank_set(7, 0xE000UL); }
static void rom_in(void)  { bank_off(5); bank_off(6); bank_off(7); }
